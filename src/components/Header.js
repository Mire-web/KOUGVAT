...
function Header({placeholder}) {
  ...
  return (
	<header className="sticky top-0 z-50 bg-white shadow-md grid grid-cols-3 p-5 md:px-10">
		{/* {logo} */}
		<div onClick={()=>router.push('/')} className={"relative cursor-pointer text-3xl text-sky-900 flex items-center h-10"}>
			<h1>KOUGVAT</h1>
		</div>
		{/* {search} */}
		<div className="flex items-center md:border-2 rounded-full py-2 md:shadow-sm">
			<input type="text" className="pl-5 flex-grow text-sm text-gray-600 placeholder-gray-400 bg-transparent outline-none" placeholder= {placeholder || "Start your search"} value={searchInput} onChange={(e)=>setSearchInput(e.target.value)}/>
		...
		<div className="flex flex-col col-span-3 mx-auto">
			...
			<div className="flex">
				...
				<button className="flex-grow text-sky-400" onClick={search}>Search</button>
			</div>
		</div>
		}
	</header>
